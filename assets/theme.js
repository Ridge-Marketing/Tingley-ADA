...
jQuery(function($) {
    ...
    var $navbar = $("#header-section"),
        $identity = $("#identity"),
        y_pos = $navbar.offset().top,
        height = $navbar.height();
  
  	var height = height  - 53;

    $(document).scroll(function() {
        var scrollTop = $(this).scrollTop();

        if (scrollTop > y_pos + height) {
            $navbar.addClass("isStuck").animate({
                top: "-150px"
            });
        } else if (scrollTop <= y_pos) {
            $navbar.removeClass("isStuck").clearQueue().animate({
                top: "-100px"
            }, 0);
        }
    });
...
theme.mobileNav = (function() {
  ...
  $('#accordion').find('.accordion-toggle').click(function(){
    ...
    //add aria-expanded to current and remove from others
    let expanded = ($(this).attr('aria-expanded') == "false") ? "true" : "false";
    $(this).attr('aria-expanded', expanded);
    $(".accordion-toggle").not($(this)).attr("aria-expanded", "false");
    $(".accordion-content").not($(this).next()).find(".accordion-toggle2").attr("aria-expanded", "false");
  ...
  $('#accordion').find('.accordion-toggle').focus().keypress(function(event) {
    if (!$(this).find('a').is(":focus") && event.keyCode == '13') {
      //add aria-expanded to current and remove from others
      let expanded = ($(this).attr('aria-expanded') == "false") ? "true" : "false";
      $(this).attr('aria-expanded', expanded);
      $(".accordion-toggle").not($(this)).attr("aria-expanded", "false");
  ...
  $('#accordion').find('.accordion-toggle2').click(function(){
    ...
    let expanded = ($(this).attr('aria-expanded') == "false") ? "true" : "false";
    $(this).attr('aria-expanded', expanded);
    $(".accordion-toggle2").not($(this)).attr("aria-expanded", "false");
  ...
  $('#accordion').find('.accordion-toggle2').focus().keypress(function(event){
    //add aria-expanded to current and remove from others
    let expanded = ($(this).attr('aria-expanded') == "false") ? "true" : "false";
    $(this).attr('aria-expanded', expanded);
    $(".accordion-toggle2").not($(this)).attr("aria-expanded", "false");
...
// var playVideo = url.searchParams.get("play");
  
//   if(playVideo){
//     $(".fancybox-media").fancybox({ padding: 0, helpers: { media: {}}}).trigger('click');
//   }
...
//   console.log('hit');
  ...
  //this is a custom function built to work with the image setup in product-template_2022
  //we want to limit # of product images shown on the page
  //this function changes thumbnail to show correct variant images on a change
  (function imageUpdate() {

    events.on("variantchange", function (variant) {

      let $productImage= $('#featuredProductImg'); //targets to hide 360 and show static featured image
      let $magic360= $('#imageContainer-2022 .MagicToolboxContainer');

      $featuredImg = variant.featured_image; //get the variant's featured image and create container for thumbnail elements
      $thumbnailOptions = null;
      if ($featuredImg) {
        
        $('#featuredProductImg').attr('src', $featuredImg.src); //set featured image and first thumbnail image to featured
        $('.thumbnail-options').first().attr('href', $featuredImg.src);
        $('.thumbnail-options').first().find('img').attr('src', $featuredImg.src);
        $thumbnailOptions = $('.thumbnail-options').slice(1); //only select thumbnails after the first since we already changed that
        $magic360.hide();
        $productImage.show();
      } else {
        $thumbnailOptions = $('.thumbnail-options'); //if there's no featured variant image, select all thumbnails for updating
      }

      $productImgArr = product.images;
      let variantImgArr = [];
      let selected_color = variant.sku.split(".")[0]; //split the SKU at the size to grab just the color option
      for (let i = 0; (i < $productImgArr.length) && (variantImgArr.length < $thumbnailOptions.length); i++) {
        //push the first few variants that have the correct color to our image array
        if ($productImgArr[i].includes(selected_color)) {
          variantImgArr.push($productImgArr[i])
        }
      }

      $thumbnailOptions.each(function(index) {
        //if link found inside image array, set thumbnail href and image src to link
        if (variantImgArr[index] != undefined) {
              $(this).attr('href', variantImgArr[index]);
              $(this).find('img').attr('src', variantImgArr[index]);
            }
      });
    });
  })();
  ...
  // The Out Of Stock dropdown function, 'check_stock()' was moved to snippet 'product-form.liquid' to prevent clash with SYNC Builder
...
// Code for the flag dropdown
var clicked = false;
var toggleDropDown = function() {
  if (clicked) {
    $('.flag-dropdown-menu').css('background', 'none');
    $('.flag-dropdown').hide();
    clicked = false;
  } else {
    $('.flag-dropdown-menu').css('background', '#383838');
    $('.flag-dropdown').show();
    clicked = true;
  }
};
...
$(document).ready(function() {
  $('.flag-dropdown-arrow').click(function() {
    toggleDropDown();
  });
  $('.flag-dropdown-menu a').click(function() {
    toggleDropDown();
  });
});

if(typeof Geolizr !== "undefined") {
  Geolizr.addEventListener("popup.show",function(popup){
    $("body").addClass("geolizr-show-popup");
  });
  Geolizr.addEventListener("popup.hide",function(popup){
    $("body").removeClass("geolizr-show-popup");
  });
}

/*============================================================================
// Configurator that isn't a configurator
//AKA the Sync System
==============================================================================*/

$(document).ready(function() {
    //Descriptions
    var $showDescription = $(".js-show-description");
    var $hideDescription = $(".js-close-description");
    var $document = $(document);

    $document.on('click', ".js-show-description", function(){
        var $this = $(this);
        $this.closest('.product-info').find('.description').removeClass('is-hidden');
        
    });

    $document.on('click', ".js-close-description", function(){
        var $this = $(this);
        $this.closest('.product-info').find('.description').addClass('is-hidden');
...
//     var selectedShells = [];
  ...
  $("[href='#build']").on('click', function(e){
    e.preventDefault();
    $('html,body').animate({scrollTop: $('#build').offset().top},'slow');
  });
  
  function getProducts(){
      $(".assembley-line-inner").addClass('show-selections');
    $(".config-product").removeClass('is-selected');
    var aTag = $("#build");
...
//       $("[data-product-handle]").hide();
      for (var key in selectedProducts) {
          if (selectedProducts.hasOwnProperty(key)) {
            for(var i = 0; i < selectedProducts[key].length; i++){
              $("[data-product-handle='" + selectedProducts[key][i] + "']").addClass('is-selected');
              if($("[data-product-handle='" + selectedProducts[key][i] + "']").find('input[type="submit"]').attr('value') == "Sold Out"){
                $(".js-add-all").prop('disabled', true);
              }
            }
          }
      }
    ...
    setTimeout(function(){
      $(".assembley-line").animate({
        height: $(".station:last-of-type").height()
      });
	    }, 100);
    }
  
  
  $(".config-product input[type='radio']").on('change', function(e){
    
    $(".js-add-all").prop('disabled', false);
    $(".is-selected[data-product-handle]").each(function(){
      if($(this).find('input[type="submit"]').attr('value') == "Sold Out"){
        $(".js-add-all").prop('disabled', true);
    	}
    });
  ...
  $(".js-next-step").on('click', function(){
    var $this = $(this);
    var curStep = $this.data('step');
    
    //Get the selected options
    var selections = $('.' + curStep).find('input[type="checkbox"]:checked');

    //Empty out the array and start fresh
    selectedProducts[curStep] = [];

    //Add it to the respective array
    selections.each(function(){
      selectedProducts[curStep].push($(this).val());
    });
    console.log(curStep, selectedProducts[curStep]);
    
    if(curStep === 'liners'){
      var stepPos = $this.data('next-step');
      $(".steps-inner").addClass('step-' + stepPos).removeClass('step-' + (stepPos - 1));
      $(".steps").animate({
        height: $(".step:last-of-type").height()
    	});
    var aTag = $("#build");
    $('html,body').animate({scrollTop: aTag.offset().top},'slow');
    }
    ...
    $(".js-prev-step").on('click', function(){
        var $this = $(this);
        var stepPos = $this.data('prev-step');

        $(".steps-inner").addClass('step-' + stepPos).removeClass('step-' + (stepPos + 1));
       $(".steps").animate({
        height: $(".step:first-of-type").height()
    	});
    var aTag = $("#build");
    $('html,body').animate({scrollTop: aTag.offset().top},'slow');
    });
  
  $(".step__products input[type='checkbox']").on('change', function(){
    var $this = $(this);
    var products = $this.closest('.step__products').find("input[type='checkbox']:checked");
    
    if(products.length > 0){
      $this.closest('.step').find('.js-next-step').prop('disabled', false);
    }
    else{
      $this.closest('.step').find('.js-next-step').prop('disabled', true);
    }
  });
  
  $(".remove-item").on('click', function(e){
  	$("[data-product-handle='" + $(this).data('product') + "']").removeClass('is-selected');
    $("input[type='checkbox'][value='" + $(this).data('product') + "']").prop('checked', false);
  });
  
  $(".back-to-start").on('click', function(){
    var $this = $(this);
      $(".steps-inner").addClass('step-1').removeClass('step-2');
      $(".steps").animate({
        height: $(".step:first-of-type").height()
      }, function(){
         $(".assembley-line").css({
    ...
    var aTag = $("#build");
    $('html,body').animate({scrollTop: aTag.offset().top},'slow');
    $(".assembley-line-inner").removeClass('show-selections');
    $('.step').each(function(){
      if($(this).find("input[type='checkbox']:checked").length == 0){
        $(this).find('.js-next-step').prop('disabled', true);
      }
    });    
  ...
  function submitForms(selected){
    if(submissionCounter < selected.length){
      
      if(submissionCounter === 0){
        selected[submissionCounter].submit();
        submissionCounter++;
        submitForms(selected);
      }
      else{
  ...
  $(".js-add-all").on('click', function(e){
    submissionCounter = 0;
    currentlySelected = [];
    var selected = $(".config-product.is-selected form");
    selected.each(function(){
      
      currentlySelected.push($(this));
    });
    
    setTimeout(function(){
      submitForms(currentlySelected);
    }, 150);
